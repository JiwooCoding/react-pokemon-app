...
async function fetchPokeDetailData(){
    try {
        ...
        const pokemonData = formatPokemonData(response.data); //포켓몬 전체 데이터 (params)
...
console.log(pokemon);
const bg = `bg-${pokemon?.type}`; //pokemon?.type => 포켓몬이 있을 때 타입을 가져오겠다는 의미
    ...
    <>
        {pokemon &&
            <Link to = {`/pokemon/${name}`}
                ...
                <div 
                    className={`${text} h=[1.5rem] text-us w-full pt-1 px-2 text-right rounded-t-lg`}
                >
